...
const Deposit: React.FC = () => {
  ...
  const handleSelectedCoin = (_coin: string) => {
    setAmounts(Object.keys(config.deployments[`netId${chainId}`][_coin].instanceAddress));
    setSelectedCoin(_coin);
  };

  const handleDeposit = async () => {
    const depositObj = { currency: selectedCoin, amount: selectedAmount, chainId: chainId };
    ...
    element.setAttribute(
      "href",
      "data:text/plain;charset=utf-8," + encodeURIComponent(note.noteString)
    );
    element.setAttribute("download", `backup-${note.noteString.substring(0, 28)}.txt`);
  ...
  function toHex(number, length = 32) {
    console.log(number);
    const str =
      number instanceof Buffer ? number.toString("hex") : snarkjs.bigInt(number).toString(16);
  ...
  function getContractInstance() {
    ...
    const tornado = new web3.eth.Contract(
      abi,
      config.deployments[`netId${chainId}`][selectedCoin].instanceAddress[selectedAmount]
    );
    return tornado;
  }
              ...
              {amounts.map((val, index) => (
                <div key={index.toString()} className="ranger">
                  <div className="input" onClick={() => setSelectedAmount(val)}>
                    <div className={val === selectedAmount ? "active" : undefined}></div>
                  </div>
                  <p>{val}</p>
                </div>
              ))}
